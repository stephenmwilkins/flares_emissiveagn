...
# H-alpha bolometric correction without power-law
conversion /= 1E44
conversion /= 10.33
conversion *=5.25E42
...
for z, c, ax in zip(redshifts, cmr.take_cmap_colors('cmr.gem_r', len(redshifts)), axes.flatten()):
    ...
    LHaStar = flares.load_dataset(tag, *['Galaxy/BPASS_2.2.1/Chabrier300/Lines/Intrinsic/HI6563/', 'Luminosity'])

    for i, (sim, w) in enumerate(zip(flares.sims, flares.weights)):

        x = np.array(LHaStar[sim])
        x = x[x>0.0]
        x = np.log10(x)
    ...
    for i, (sim, w) in enumerate(zip(flares.sims, flares.weights)):
        ...
        x = np.array(LHaStar[sim]) + np.array(BH_Mdot[sim]) * conversion
        x = x[x>0.0]
        x = np.log10(x)
...
fig.text(left+(right-left)/2, 0.08, r'$\rm \log_{10}[L(H\alpha)/erg\ s^{-1}]$', ha='center', fontsize = 9)
