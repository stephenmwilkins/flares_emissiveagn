...
bin_edges = np.arange(*X_limits[::-1], binw)
...
for z, c, ax in zip(redshifts, cmr.take_cmap_colors('cmr.bubblegum_r', len(redshifts)), axes.flatten()):
    ...
    BH_los = flares.load_dataset(tag, *['Galaxy', 'BH_los'])
    DTM = flares.load_dataset(tag, *['Galaxy', 'DTM'])

    for i, (sim, w) in enumerate(zip(flares.sims, flares.weights)):

        x = np.array(BH_Mdot[sim]) * conversion # intrinsic UV luminosity
        tau = kappa * DTM[sim] * BH_los[sim] * (1500./5500.)**gamma
        x *= np.exp(-tau)
        x = Lnu_to_M(x)
    ...
    LUVStar = flares.load_dataset(tag, *['Galaxy/BPASS_2.2.1/Chabrier300/Luminosity/DustModelI', 'FUV'])
    ...
    for i, (sim, w) in enumerate(zip(flares.sims, flares.weights)):
        ...
        stars = np.array(LUVStar[sim])
        bhs = np.array(BH_Mdot[sim]) * conversion # intrinsic UV luminosity
        tau = kappa * DTM[sim] * BH_los[sim] * (1500./5500.)**gamma
        bhs *= np.exp(-tau)
    ...
    if z==5:
        ...
        ax.errorbar(x, y, xerr=xerr, yerr=yerr, fmt="o", c='k', markersize=5, label=r'$\rm Maiolino+23$')
    ...
    if z==5:
        ...
        ax.errorbar(x, y, yerr=yerr, fmt="D", c='k', markersize=5, label=r'$\rm Kocevski+23$')
    ...
    if z==5:
        ...
        ax.errorbar(x, y, yerr=yerr, fmt="s", c='k', markersize=5, label=r'$\rm Matthee+23$')
