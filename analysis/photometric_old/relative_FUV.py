...
conversion *= 0.2 # 1/bolometric correction
conversion /= c/(1500*Angstrom)
log10conversion = np.log10(conversion.to('erg/s/Hz').value)
...
for z, c, ax in zip(redshifts, cmr.take_cmap_colors('cmr.bubblegum_r', len(redshifts)), axes.flatten()):
    ...
    log10Lstars = D['log10FUV_Intrinsic']
...
fig.text(0.04, bottom+(top-bottom)/2, r'$\rm \log_{10}(L_{\bullet,\ FUV}/L_{\star,\ FUV})$', rotation = 90, va='center', fontsize = 9)
fig.text(left+(right-left)/2, 0.08, r'$\rm\log_{10}(L_{\bullet,\ FUV}+L_{\star,\ FUV}/erg\ s^{-1}\ Hz^{-1})$', ha='center', fontsize = 9)
...
fig.savefig(f'figs/relative_LFUV.pdf')
fig.savefig(f'figs/relative_LFUV.png')
